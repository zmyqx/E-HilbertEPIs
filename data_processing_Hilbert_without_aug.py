...
def load_data(cell_line):
    ...
    with h5py.File(data_path, 'r') as hf:
        enhancers = np.array(
            hf.get(cell_line + '_X_enhancers')).transpose((0, 2, 1))
        promoters = np.array(
            hf.get(cell_line + '_X_promoters')).transpose((0, 2, 1))
...
dataset_enhancer = np.zeros((len(new_enhancers_all), 64, 64, 4))
dataset_promoter = np.zeros((len(new_promoters_all), 64, 64, 4))
...
# 将正负样本转化为三维图像
sample_count = 0
for k in range(len(new_enhancers_all)):
    if 'N' not in new_enhancers_all[k] and 'N' not in new_promoters_all[k]:
        dataset_enhancer[k] = make_image(new_enhancers_all[k])
        dataset_promoter[k] = make_image(new_promoters_all[k])
        sample_count += 1
...
file_name_hilbert_all = './data_Hilbert/' + \
                        cell + '_all_data_hilbert_without_aug.h5'
