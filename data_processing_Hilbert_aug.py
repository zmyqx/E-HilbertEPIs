...
# 然后，扩展正样本数量（随机移动前n个序列到末尾）
# 接着，将增强子(3000,4)和启动子(2000,4)数据通过hilbert编码（64*64*4）
# 最后，将编码结果以.h5的形式保存
###

cell = 'HeLa-S3'  # HeLa-S3  HUVEC
...
# file_name_hilbert = './data_Hilbert/K562fantom_point_list.txt'
...
for k in range(len(aug_enhancers_all)):
    if 'N' not in aug_enhancers_all[k] and 'N' not in aug_promoters_all[k]:
